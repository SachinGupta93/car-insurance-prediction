...
class MultiGeminiCarDamageRAG:
    def __init__(self):
        ...
        # Request throttling - REDUCED for faster response
    ...
    def analyze_with_retry(self, image_path, max_retries=None):
        ...
        if max_retries is None:
            max_retries = min(len(self.api_keys), 3)  # Try max 3 keys only for speed
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Add rate limiting delay to prevent rapid requests - REDUCED for speed
                if attempt > 0:
                    delay = min(0.5 * attempt, 2)  # Linear backoff, max 2 seconds
                ...
                if any(keyword in error_msg for keyword in ['quota', 'limit', 'exceeded', '429', 'rate', 'resource_exhausted']):
                    ...
                    # Add delay after quota exceeded to prevent hammering - REDUCED
                    time.sleep(0.5)
                    continue
                else:
