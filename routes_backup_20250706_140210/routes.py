...
@api.route('/user/history/add', methods=['POST'])
@firebase_auth_required
def add_analysis_to_history():
    """Add analysis to user's history"""
    try:
        from flask import current_app
        logger.info(f"📝 add_analysis_to_history: Request received from user {request.user.get('uid', 'unknown')}")
        
        try:
            # Check if db_ref exists in app config
            if 'db_ref' not in current_app.config:
                logger.error("❌ add_analysis_to_history: No database reference in app config")
                return jsonify({'error': 'Database reference not configured', 'success': False}), 500
            ...
            # Make sure user ID is available
            if not request.user or 'uid' not in request.user:
                logger.error("❌ add_analysis_to_history: No user ID available in request")
                return jsonify({'error': 'User not authenticated', 'success': False}), 401
            ...
            if not analysis_data:
                logger.error("❌ add_analysis_to_history: No analysis data provided")
                return jsonify({'error': 'Analysis data is required', 'success': False}), 400
            
            logger.info(f"📊 add_analysis_to_history: Adding analysis for user {user_id}")
            ...
            # Add analysis to user's history
            result = user_auth.add_analysis_history(user_id, analysis_data)
            logger.info(f"✅ add_analysis_to_history: Analysis saved successfully")
            return jsonify({'success': True, 'id': result if result else 'saved', 'message': 'Analysis saved to history'}), 200
            
        except Exception as save_error:
            logger.error(f"❌ add_analysis_to_history: Error saving to history: {str(save_error)}")
            logger.error(f"📚 add_analysis_to_history: Error traceback: {traceback.format_exc()}")
            return jsonify({'error': f'Failed to save analysis: {str(save_error)}', 'success': False}), 500
            
    except Exception as e:
        logger.error(f"💥 add_analysis_to_history: Error occurred: {str(e)}")
        logger.error(f"📚 add_analysis_to_history: Error traceback: {traceback.format_exc()}")
        return jsonify({'error': str(e), 'success': False}), 500
